...
class UpdatePublisher extends React.Component {
    ...
    componentDidMount() {
        axios
            .get(
                `http://localhost:8080/lms/viewPublishersbyId/${this.props.match.params.id}`
            )
            .then((res) => {
    ...
    handleChange = (event) => {
        const publisher = { ...this.state.publisher };
        publisher[event.target.name] = event.target.value;
    ...
    handleSubmit = (event) => {
        ...
        axios
            .put("http://localhost:8080/lms/updatePublisher", publisher)
            .then((res) => {
